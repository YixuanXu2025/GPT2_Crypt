...
GELU_DEGREE = int(os.environ.get("GELU_DEGREE", "7"))   # 建议 6/7/8，且 <=8
GELU_CENTRAL_MASS = float(os.environ.get("GELU_CENTRAL_MASS", "0.90"))
GELU_CALIB_BATCHES = int(os.environ.get("GELU_CALIB_BATCHES", "50"))
GELU_MAX_SAMPLES = int(os.environ.get("GELU_MAX_SAMPLES", "2000000"))
...
raw = DatasetDict({
    "train": raw["train"].select(range(250000)),
...
def calibrate_ln_stats(model, dataset, data_collator, device, batch_size: int, num_batches: int):
    ...
    def make_hook(name: str, eps: float):
        def hook(mod, inputs, output):
            ...
            # 累计到 CPU，节省显存
            sums_mu[name] += mu.detach().cpu()
            sums_invstd[name] += inv_std.detach().cpu()
            counts[name] += 1
        return hook
    ...
    for name, mod in model.named_modules():
        if isinstance(mod, nn.LayerNorm):
            ...
            if isinstance(shape, int):
                shape = (shape,)
            H = int(shape[-1])
            sums_mu[name] = torch.zeros(H, dtype=torch.float32)
            sums_invstd[name] = torch.zeros(H, dtype=torch.float32)
            counts[name] = 0
...
def apply_he_approx(model, gelu_approx="poly", ln_approx="static_calib", ln_stats=None):
        ...
        for name, child in list(parent.named_children()):
            ...
            if isinstance(child, nn.LayerNorm):
                ...
                if LN_SCOPE == "ln_2_only" and not full_name.endswith("ln_2"):
                    continue
                # 渐进替换 ln_1：当 LN_SCOPE=all 且 LN1_TOPK>0 时，只替换最后 K 个 block 的 ln_1
                if LN_SCOPE == "all" and LN1_TOPK > 0 and full_name.endswith("ln_1") and hasattr(model, "config") and hasattr(model.config, "n_layer"):
                    import re
                ...
                if ln_approx == "static_calib" and ln_stats is not None and full_name in ln_stats:
                    ...
                    # 这里是 shape=(H,) 的向量；确保 dtype 为 fp32，后续会随 model.to(device) 迁移
                    mu = mu_vec.to(dtype=torch.float32)
                    inv_std = inv_std_vec.to(dtype=torch.float32)
    ...
    def _is_gelu_like(obj) -> bool:
        # 1) 纯 PyTorch GELU
        if isinstance(obj, nn.GELU):
            return True
        # 2) Transformers 的 NewGELUActivation / GELUActivation / FastGELU... 等，靠类名兜底最稳
        cls = obj.__class__.__name__.lower()
        if "gelu" in cls:
            return True
        # 3) 极少数情况下 act 可能是函数（callable），也兜底
        if callable(obj) and (not isinstance(obj, nn.Module)):
            fn = getattr(obj, "__name__", "").lower()
            if "gelu" in fn:
                return True
        return False
    ...
    def _replace_gelu(parent: nn.Module, prefix: str = ""):
        ...
        for name, child in list(parent.named_children()):
            ...
            if _is_gelu_like(child):
                ...
                gelu_replaced_names.append(full_name)
            else:
                _replace_gelu(child, full_name)

        # 特判：很多 GPT2 MLP 的 act 是属性，不一定出现在 named_children 里
        if parent.__class__.__name__.lower().endswith("mlp") and hasattr(parent, "act"):
            act = getattr(parent, "act")
            if _is_gelu_like(act):
                setattr(parent, "act", HEGELU(approx=gelu_approx))
                gelu_replaced_names.append(f"{prefix}.act" if prefix else "act")

    if REPLACE_LN:
        _replace_ln(model)
    if REPLACE_GELU:
        _replace_gelu(model, prefix="")
        print(f"[GELU REPLACED] count={len(gelu_replaced_names)}")
        for n in gelu_replaced_names[:50]:
            print("  -", n)
        if len(gelu_replaced_names) > 50:
            print("  ...")
...
args = TrainingArguments(
    ...
    # 评估和保存间隔调整为每10万步
    eval_strategy="steps",
    eval_steps=1000,  # 每10万步评估一次
    save_strategy="steps",
    save_steps=2000,  # 每10万步保存一次
